    ...
    def save(self):
        ...
        if self.checkBox_add_triangles.checkState() == Qt.Checked:
            for i in range(t_train):
                ...
                cv2.imwrite(path + "/trainA/t%d.png" % i, image)
                cv2.imwrite(path + "/trainB/t%d.png" % i, cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))
            for i in range(t_test):
                image = self.gen_triang(color=tuple(list(self.triangles_color)[::-1]), center=self.triangles_center)
                cv2.imwrite(path + "/testA/t%d.png" % i, image)
                cv2.imwrite(path + "/testB/t%d.png" % i, cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))
        if self.checkBox_add_circles.checkState() == Qt.Checked:
            for i in range(c_train):
                image = self.gen_circle(color=tuple(list(self.circles_color)[::-1]), center=self.circles_center)
                cv2.imwrite(path + "/trainA/c%d.png" % i, image)
                cv2.imwrite(path + "/trainB/c%d.png" % i, cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))
            for i in range(c_test):
                image = self.gen_circle(color=tuple(list(self.circles_color)[::-1]), center=self.circles_center)
                cv2.imwrite(path + "/testA/c%d.png" % i, image)
                cv2.imwrite(path + "/testB/c%d.png" % i, cv2.cvtColor(image, cv2.COLOR_BGR2GRAY))
